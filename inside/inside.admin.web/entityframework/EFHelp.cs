...
namespace inside.admin.web.entityframework
{
    public class EFHELP
    {
        ...
        public EFHELP()
        {
            strConn = SQLString.defaultDBString;
        }

        public EFHELP(string dbString)
        {
            strConn = dbString;
        }
        ...
        public void LookError<T>(Expression<Func<T, bool>> seleWhere) where T : class
        {
            try
            {
                using (EFDataContext<T> db = new EFDataContext<T>(strConn))
                {
            ...
            using (EFDataContext db = new EFDataContext(strConn))
            {
                using (var conn = new SqlConnection(strConn))
                {
                    ...
                    using (SqlDataAdapter sqlAd = new SqlDataAdapter(sql, conn))
                    {
                        ...
                        sqlAd.Fill(table);
                        sqlAd.SelectCommand.Parameters.Clear();
                    }
                }
            }
