...
namespace inside.crsri.Dal
{
    public class homeDal : baseDal
    {
            ...
            string sql = string.Format(@"select subjectID,subjectName,articleID,title,keywords,convert(varchar(5),updateTime,10) as 'releaseTime',isTop  from (
					...
					select b.subjectID,b.subjectName,articleID,title,keywords,updateTime,isTop from (
						...
						SELECT *,ROW_NUMBER() OVER(PARTITION BY subjectID ORDER BY isTop DESC,updateTime DESC) NUM FROM Article where  isDeleted=0 AND isPassed=1 and
								...
								) as a   join [Subject] as b on a.subjectID=b.subjectID where NUM<=18
        ...
        public static requestHomeArticleClass reqHomeEnglishNewsArticle()
        {
            ...
            string sql = @"select TOP 5 subjectID,articleID,title,keywords,convert(varchar(5),releaseTime,10) as 'releaseTime' from Article where subjectID = '018003' and isDeleted=0 and isPassed=1  ORDER BY articleID DESC";
            DataTable dtSource = CrsriEntityFramework.QueryDataTable(sql);
        ...
        public static List<requestHomeArticleClass> requestHomeSpecial()
        {
            ...
            foreach (DataRow dr in dtSource.Rows)
            {
                special.Add(new reqSpecialClass() { specialID = dr["specialID"].ToString(), specialName = dr["specialName"].ToString(), linkUrl = dr["linkUrl"].ToString() });

                exeSql.Add(string.Format(@"
                               SELECT * FROM (								
							   select top 12 subjectID,articleID,title,a.specialID,b.specialName 'keywords',convert(varchar(5),a.updateTime,10) as 'releaseTime',isTop from Article as a 
        ...
        #region 首页-通栏图片
        public static List<t_web_siteCfgPic_model> cfgPicList(int position = 0)
        {

            List<OrderModelField> order = new List<OrderModelField>();
            order.Add(new OrderModelField() { PropertyName="id",IsDesc=true });
            if (position == 0)
            {
                return CrsriEntityFramework.GetAll<t_web_siteCfgPic_model>(order.ToArray());
            }
            else
            {
                List<ExpressionModelField> listWhere = new List<ExpressionModelField>();
                listWhere.Add(new ExpressionModelField() { Name = "position", Value = position });
                return CrsriEntityFramework.GetList<t_web_siteCfgPic_model>(listWhere.ToArray(),order.ToArray());
            }
        ...
        public static List<t_web_subject_model> reqHomeQualManage()
        {
            string sql = @"select top 8 * from Subject where subjectID like '005%' and LEN(subjectID)=6 and subjectID<> 005002 order by CONVERT(int,subjectID) asc";
