...
{
    public class expcolDal : baseDal
    {


        public static requestHomeArticleClass reqExpCol()
        {
            requestHomeArticleClass req = new requestHomeArticleClass();
            string sql = @"select articleID,title,keywords,convert(varchar(5),releaseTime,10) as 'releaseTime' from Article where specialID like '038%' and isDeleted=0 and isPassed=1  ORDER BY releaseTime DESC";
